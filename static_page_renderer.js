...
function getRelatedGuides(persona) {
  const all = [
    ...
    { label: 'Rodrigues Island Guide',                 slug: 'rodrigues-island-mauritius',              persona: null           },
...
function generateHead(meta, baseUrl, siteName, lang, schemaScripts) {
  ...
  const lines = [
    ...
    `  <!-- Theme: anti-FOUC + global toggle (runs before paint) -->`,
    `  <script>!function(){var K='mrf-theme',H=document.documentElement;function set(t){H.setAttribute('data-theme',t);try{localStorage.setItem(K,t)}catch(e){}}function toggle(){set(H.getAttribute('data-theme')==='dark'?'light':'dark');}var s=localStorage.getItem(K)||(matchMedia('(prefers-color-scheme:dark)').matches?'dark':'light');H.setAttribute('data-theme',s);window.mrfToggle=toggle;window.mrfSetTheme=set;}()</script>`,
    `  <!-- Google Analytics -->`,
    ...
    `    :root{--color-bg:#F8FAFC;--color-surface:#fff;--color-primary:#0F766E;--color-primary-hov:#0D6B63;--color-tx:#1E293B;--color-tx-muted:#64748B}`,
    `    [data-theme="dark"]{--color-bg:#020617;--color-surface:#0A1628;--color-primary:#2DD4BF;--color-primary-hov:#26B8A6;--color-tx:#F1F5F9;--color-tx-muted:#94A3B8}`,
    /* ── Light mode legacy overrides ────────────────────────────────────── */
    `    [data-theme="light"]{--navy-card:#fff;--navy-raised:#f5f1e8;--midnight:#f0ece2;--champagne:#1a1108;--text:#1E293B;--text-dim:#4a3929;--muted:#64748B;--gold:#b8902d;--gold-dim:#966a1a;--gold-bright:#c9a84c;--gold-glow:rgba(184,144,45,.10);--gold-glow-lg:rgba(184,144,45,.05);--border:rgba(0,0,0,.08);--border-light:rgba(0,0,0,.13);--border-gold:rgba(184,144,45,.35);--border-gold-strong:rgba(184,144,45,.55);--shadow-card:0 2px 20px rgba(0,0,0,.07);--shadow-hover:0 8px 40px rgba(0,0,0,.12)}`,
    `    body{background:var(--color-bg)}`,
    `    [data-theme="light"] body{color:var(--text)}`,
    `    [data-theme="light"] .site-header{background:rgba(248,250,252,.97)}`,
    `    [data-theme="light"] .hotel-card{background:var(--navy-card);border-color:var(--border)}`,
    `    [data-theme="light"] .hotel-card__score-track{background:rgba(0,0,0,.08)}`,
    `    [data-theme="light"] .comparison__corner{background:#f5f1e8}`,
    `    [data-theme="light"] .comparison__table tbody tr:nth-child(even){background:var(--navy-card)}`,
    /* Smooth transitions */
    `    body,.site-header,.hotel-card,.ranking-summary,.methodology,.faq__item,.disclosure,.affiliate-cta,.comparison{transition:background-color .3s ease,border-color .3s ease,color .2s ease}`,
    `    @media(prefers-reduced-motion:reduce){body,.site-header,.hotel-card{transition:none}}`,
    /* ── Pill toggle ─────────────────────────────────────────────────────── */
    `    .theme-toggle{position:relative;width:50px;height:28px;border-radius:14px;border:none;background:rgba(184,144,45,.22);cursor:pointer;padding:4px;flex-shrink:0;transition:background .3s ease}`,
    `    [data-theme="dark"] .theme-toggle{background:rgba(45,212,191,.20)}`,
    `    .theme-toggle:hover{background:rgba(184,144,45,.38)}`,
    `    [data-theme="dark"] .theme-toggle:hover{background:rgba(45,212,191,.35)}`,
    `    .theme-toggle:focus-visible{outline:2px solid var(--gold);outline-offset:3px}`,
    `    .theme-toggle__track{display:block;position:relative;width:100%;height:100%}`,
    `    .theme-toggle__thumb{position:absolute;top:0;left:0;width:20px;height:20px;border-radius:50%;background:#fff;display:flex;align-items:center;justify-content:center;transition:transform .35s cubic-bezier(.34,1.56,.64,1);box-shadow:0 1px 4px rgba(0,0,0,.25)}`,
    `    [data-theme="dark"] .theme-toggle__thumb{transform:translateX(22px)}`,
    `    .theme-toggle__sun,.theme-toggle__moon{position:absolute;transition:opacity .2s ease;line-height:0}`,
    `    .theme-toggle__sun{color:#b8902d}.theme-toggle__moon{color:#0F766E}`,
    `    [data-theme="dark"] .theme-toggle__sun{opacity:0}[data-theme="dark"] .theme-toggle__moon{opacity:1}`,
    `    [data-theme="light"] .theme-toggle__moon{opacity:0}[data-theme="light"] .theme-toggle__sun{opacity:1}`,
    `    :not([data-theme]) .theme-toggle__sun{opacity:1}:not([data-theme]) .theme-toggle__moon{opacity:0}`,
    `    @media(prefers-reduced-motion:reduce){.theme-toggle__thumb{transition:transform .15s ease}.theme-toggle__sun,.theme-toggle__moon{transition:none}}`,
...
function generateSiteHeader(siteName, baseUrl) {
  return [
    ...
    `    </ul>`,
    `    <button id="theme-toggle" class="theme-toggle" type="button" aria-label="Switch to dark mode" aria-pressed="false" onclick="mrfToggle()">`,
    `      <span class="theme-toggle__track" aria-hidden="true">`,
    `        <span class="theme-toggle__thumb">`,
    `          <svg class="theme-toggle__sun" width="11" height="11" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2.5" stroke-linecap="round" stroke-linejoin="round"><circle cx="12" cy="12" r="5"/><path d="M12 2v2M12 20v2M4.22 4.22l1.42 1.42M18.36 18.36l1.42 1.42M2 12h2M20 12h2M4.22 19.78l1.42-1.42M18.36 5.64l1.42-1.42"/></svg>`,
    `          <svg class="theme-toggle__moon" width="11" height="11" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2.5" stroke-linecap="round" stroke-linejoin="round"><path d="M21 12.79A9 9 0 1 1 11.21 3 7 7 0 0 0 21 12.79z"/></svg>`,
    `        </span>`,
    `      </span>`,
    `    </button>`,
    `  </nav>`,
