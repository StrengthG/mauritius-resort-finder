...
function generateHead(meta, baseUrl, siteName, lang, schemaScripts) {
  ...
  const lines = [
    ...
    `  <script>!function(){var K='mrf-theme',H=document.documentElement;function set(v){H.setAttribute('data-theme',v);try{localStorage.setItem(K,v)}catch(e){}}function toggle(){set(H.getAttribute('data-theme')==='light'?'dark':'light');}var s=localStorage.getItem(K)||(matchMedia('(prefers-color-scheme:light)').matches?'light':'dark');set(s);window.mrfToggle=toggle;}()</script>`,
    `  <!-- Google Analytics -->`,
    ...
    `    [data-theme="light"]{--navy-card:#ffffff;--navy-raised:#f5f1e8;--midnight:#f0ece2;--champagne:#1a1108;--text:#2d2010;--text-dim:#4a3929;--muted:#70655a;--gold:#b8902d;--gold-dim:#966a1a;--gold-bright:#c9a84c;--gold-glow:rgba(184,144,45,.10);--gold-glow-lg:rgba(184,144,45,.05);--border:rgba(0,0,0,.08);--border-light:rgba(0,0,0,.13);--border-gold:rgba(184,144,45,.35);--border-gold-strong:rgba(184,144,45,.55);--shadow-card:0 2px 20px rgba(0,0,0,.07);--shadow-hover:0 8px 40px rgba(0,0,0,.12)}`,
    `    [data-theme="light"] body{background:#faf8f4;color:var(--text)}`,
    `    [data-theme="light"] .site-header{background:rgba(250,248,244,.97)}`,
    `    [data-theme="light"] .hero{background:#faf8f4}.hero::before{background:radial-gradient(ellipse 80% 50% at 50% -5%,rgba(184,144,45,.07) 0%,transparent 65%)}`,
    ...
    /* ── Theme Toggle ────────────────────────────────────────────────────── */
    `    .theme-toggle{position:relative;display:inline-flex;align-items:center;justify-content:center;width:32px;height:32px;border-radius:50%;border:1px solid var(--border-gold);background:var(--gold-glow);cursor:pointer;padding:0;color:var(--gold);flex-shrink:0;transition:background .3s ease,border-color .3s ease,transform .25s cubic-bezier(.34,1.56,.64,1),color .2s ease}`,
    `    .theme-toggle:hover{background:rgba(201,168,76,.18);border-color:var(--border-gold-strong)}`,
    `    [data-theme="light"] .theme-toggle:hover{background:rgba(184,144,45,.15)}`,
    `    .theme-toggle:focus-visible{outline:2px solid var(--gold);outline-offset:3px}`,
    `    .theme-toggle--pressed{transform:scale(.88)!important}`,
    `    .theme-toggle__icon{position:absolute;transition:opacity .3s ease,transform .4s cubic-bezier(.34,1.56,.64,1)}`,
    `    [data-theme="dark"] .theme-toggle__icon--sun{opacity:1;transform:rotate(0deg) scale(1)}`,
    `    [data-theme="dark"] .theme-toggle__icon--moon{opacity:0;transform:rotate(-30deg) scale(.4);pointer-events:none}`,
    `    [data-theme="light"] .theme-toggle__icon--moon{opacity:1;transform:rotate(0deg) scale(1)}`,
    `    [data-theme="light"] .theme-toggle__icon--sun{opacity:0;transform:rotate(45deg) scale(.4);pointer-events:none}`,
    `    :not([data-theme]) .theme-toggle__icon--sun{opacity:1}:not([data-theme]) .theme-toggle__icon--moon{opacity:0}`,
    `    @media(prefers-reduced-motion:reduce){.theme-toggle__icon{transition:opacity .15s ease}}`,
...
function generateSiteHeader(siteName, baseUrl) {
  return [
    ...
    `    </ul>`,
    `    <button id="theme-toggle" class="theme-toggle" type="button" aria-label="Switch to light mode" onclick="mrfToggle()">`,
    `      <svg class="theme-toggle__icon theme-toggle__icon--sun" width="15" height="15" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2.2" stroke-linecap="round" stroke-linejoin="round" aria-hidden="true"><circle cx="12" cy="12" r="4.5"/><path d="M12 2v2M12 20v2M4.22 4.22l1.42 1.42M18.36 18.36l1.42 1.42M2 12h2M20 12h2M4.22 19.78l1.42-1.42M18.36 5.64l1.42-1.42"/></svg>`,
    `      <svg class="theme-toggle__icon theme-toggle__icon--moon" width="14" height="14" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2.2" stroke-linecap="round" stroke-linejoin="round" aria-hidden="true"><path d="M21 12.79A9 9 0 1 1 11.21 3 7 7 0 0 0 21 12.79z"/></svg>`,
    `    </button>`,
    `  </nav>`,
    `</header>`,
  ].join('\n');
